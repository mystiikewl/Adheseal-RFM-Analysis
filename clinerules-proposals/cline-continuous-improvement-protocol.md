...
- **Identify Learnings:** What new information, techniques, **underlying patterns,** API behaviors, project-specific commands (e.g., test, build, run flags), environment variables, setup quirks, or successful outcomes were discovered? **What core principles can be extracted?** Consider if any structured, queryable information should be stored in the MCP memory server.
...
- **Note on Format**: Each entry should include `Category` and `Priority` fields to aid in filtering and organizing learnings for consolidation. Categories could include "Authentication", "Workflow", "Data Processing", etc., while Priority can be "High", "Medium", or "Low" based on the impact of the learning.
